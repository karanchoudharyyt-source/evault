...
function sig(ev:number){
  if(ev>=1.3)return{label:"GREAT VALUE",c:"#00ff88",bg:"rgba(0,255,135,.12)",bd:"rgba(0,255,135,.28)"};
...
function dec(pack:Pack){
  ...
  if(bb>=1.2)return{action:"🔥 STRONG BUY",c:"#00ff88",bg:"rgba(0,255,135,.1)",reason:`Buyback EV ${(bb*100).toFixed(0)}% — you statistically profit after ALL fees`};
  if(bb>=1.0)return{action:"✅ BUY",c:"#00ff88",bg:"rgba(0,255,135,.08)",reason:`Real cash expected to exceed pack price after all fees`};
...
function evc(ev:number){return ev>=1.3?"#00ff88":ev>=1.0?"#4fd8a0":ev>=0.9?"#ffd166":"#ff3860";}
...
function UpgradeModal({onClose}:{onClose:()=>void}){
  ...
  const M={fontFamily:"'Space Mono',monospace"} as React.CSSProperties;
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:"12px"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#0a0e16",border:"1px solid #1a2f45",borderRadius:16,width:"100%",maxWidth:400,maxHeight:"92vh",overflowY:"auto",position:"relative"}}>

        {/* ── Header row ── */}
        <div style={{padding:"16px 18px 12px",borderBottom:"1px solid #0e1e30",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,background:"#0a0e16",zIndex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{width:6,height:6,borderRadius:"50%",background:"#00ff88",display:"inline-block"}}/>
            <span style={{fontSize:10,color:"#00ff88",fontWeight:700,...M,letterSpacing:.5}}>PACKPULSE PRO</span>
          </div>
          <button onClick={onClose} style={{all:"unset" as any,cursor:"pointer",color:"rgba(255,255,255,.3)",fontSize:16,lineHeight:1}}>✕</button>
        </div>

        {/* ── Price + bar ── */}
        <div style={{padding:"14px 18px",borderBottom:"1px solid #0e1e30"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            <span style={{fontWeight:800,fontSize:38,color:"#00ff88",lineHeight:1,...M}}>${Math.round(price)}</span>
            <div>
              <div style={{fontSize:11,color:"rgba(255,255,255,.3)"}}>/month</div>
            ...
            {isFounding&&<div style={{marginLeft:"auto",background:"rgba(255,209,102,.08)",border:"1px solid rgba(255,209,102,.2)",borderRadius:8,padding:"5px 10px",textAlign:"center" as const}}>
              <div style={{fontSize:8,color:"rgba(255,255,255,.3)",...M}}>NEXT PRICE</div>
          ...
          {isFounding&&(
            <>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:9,marginBottom:4,...M}}>
                <span style={{color:"rgba(255,255,255,.3)"}}>{filled}/100 early access spots claimed</span>
              ...
              <div style={{height:4,background:"#122038",borderRadius:2,overflow:"hidden"}}>
                <div style={{height:"100%",width:`${pct}%`,background:"#00ff88",borderRadius:2}}/>
              </div>
            </>
          )}
        </div>
        ...
        <div style={{padding:"12px 18px",borderBottom:"1px solid #0e1e30"}}>
          <div style={{fontSize:8,color:"rgba(255,255,255,.3)",letterSpacing:1.5,...M,marginBottom:8}}>WHAT YOU UNLOCK</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
            {[
              ...
              <div key={i} style={{background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",borderRadius:8,padding:"8px 10px",display:"flex",gap:7,alignItems:"flex-start"}}>
                <span style={{fontSize:14,flexShrink:0}}>{f.icon}</span>
                <div style={{minWidth:0}}>
                  <div style={{fontWeight:700,fontSize:10,color:"#fff",marginBottom:1}}>{f.name}</div>
                  <div style={{fontSize:9,color:"rgba(255,255,255,.3)",lineHeight:1.3}}>{f.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{padding:"14px 18px"}}>
          {user?(
            <button onClick={handleUpgrade} disabled={loading}
              style={{width:"100%",padding:"13px",background:loading?"#1a2a3a":"#00ff88",color:"#000",border:"none",borderRadius:10,fontWeight:800,fontSize:14,cursor:loading?"not-allowed":"pointer",...M,marginBottom:10}}>
            ...
            <SignInButton mode="modal" forceRedirectUrl="/?upgrade=1">
              <button style={{width:"100%",padding:"13px",background:"#00ff88",color:"#000",border:"none",borderRadius:10,fontWeight:800,fontSize:14,cursor:"pointer",...M,marginBottom:10}}>
                Sign In to Upgrade →
              </button>
            </SignInButton>
          )}
          <div style={{display:"flex",justifyContent:"space-between",fontSize:8,color:"rgba(255,255,255,.3)",...M}}>
  ...
  return(
    <div onClick={onUpgrade} style={{cursor:"pointer",position:"relative",filter:"blur(2px)",pointerEvents:"none",userSelect:"none" as const}}>
      ...
      <div style={{position:"absolute",inset:0,background:"rgba(6,13,24,.7)",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:8,pointerEvents:"all"}}>
        <span style={{fontSize:10,color:"#00ff88",fontWeight:700,fontFamily:"'Space Mono',monospace"}}>🔒 PRO</span>
      </div>
    </div>
  );
}
...
body{background:#060608!important;color:#e8edf5;font-family:'DM Sans',system-ui,sans-serif;-webkit-font-smoothing:antialiased}
::-webkit-scrollbar{width:3px;height:3px}::-webkit-scrollbar-thumb{background:#1a2a3a;border-radius:2px}

/* LIVE DOT */
.ld{display:inline-block;width:6px;height:6px;border-radius:50%;background:#00ff88;animation:lp 2s infinite}
@keyframes lp{0%,100%{opacity:1}50%{opacity:.25}}

/* TICKER */
.ticker-wrap{background:#0a0e16;border-bottom:1px solid rgba(255,255,255,.06);height:32px;display:flex;align-items:center;overflow:hidden;flex-shrink:0}
.ticker-lbl{padding:0 14px;font-size:9px;font-weight:700;color:#00ff88;letter-spacing:2px;font-family:'Space Mono',monospace;border-right:1px solid rgba(255,255,255,.06);height:100%;display:flex;align-items:center;gap:6px;flex-shrink:0;white-space:nowrap}
.ticker-track{display:flex;white-space:nowrap;animation:tick 60s linear infinite}
.ticker-item{display:inline-flex;align-items:center;gap:8px;padding:0 18px;font-size:10px;border-right:1px solid rgba(255,255,255,.04);height:32px;font-family:'Space Mono',monospace;flex-shrink:0}
@keyframes tick{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}

/* NAV */
.nav{height:48px;display:flex;align-items:center;padding:0 16px;gap:10px;border-bottom:1px solid rgba(255,255,255,.06);background:#0a0e16;flex-shrink:0}
.hstat{padding:4px 12px;border:1px solid rgba(255,255,255,.07);border-radius:6px;text-align:center;flex-shrink:0;background:rgba(255,255,255,.02)}
.hstat-l{font-size:7px;color:rgba(255,255,255,.3);letter-spacing:1.5px;font-family:'Space Mono',monospace;text-transform:uppercase}
.hstat-v{font-weight:700;font-size:14px;line-height:1.3;font-family:'Syne',sans-serif}

/* SIDEBAR */
.sb-btn{all:unset;box-sizing:border-box;display:flex;align-items:center;gap:8px;padding:8px 14px;font-size:11px;font-weight:500;border-left:2px solid transparent;cursor:pointer;color:rgba(255,255,255,.35);width:100%;transition:all .15s;overflow:hidden;letter-spacing:.01em}
.sb-btn:hover{color:rgba(255,255,255,.75);background:rgba(255,255,255,.03)}
.sb-btn.on{color:#e8edf5;border-left-color:#00ff88;background:rgba(0,255,136,.04)}

/* CAT TABS */
.cat-tab{padding:5px 13px;border-radius:20px;font-size:10px;font-weight:600;cursor:pointer;border:1px solid rgba(255,255,255,.08);background:transparent;color:rgba(255,255,255,.35);font-family:'Space Mono',monospace;transition:all .15s;white-space:nowrap;letter-spacing:.02em}
.cat-tab:hover{color:rgba(255,255,255,.7);border-color:rgba(255,255,255,.15)}
.cat-tab.on{background:rgba(0,255,136,.1);border-color:rgba(0,255,136,.35);color:#00ff88}

/* TOOLBAR BUTTONS */
.tb{background:transparent;border:1px solid rgba(255,255,255,.08);border-radius:5px;padding:4px 10px;font-size:9px;color:rgba(255,255,255,.35);cursor:pointer;font-family:'Space Mono',monospace;transition:all .15s;font-weight:700;letter-spacing:.05em}
.tb:hover{color:rgba(255,255,255,.7);border-color:rgba(255,255,255,.18)}
.tb.on{color:#00ff88;border-color:rgba(0,255,136,.35);background:rgba(0,255,136,.07)}

/* SIGNAL BADGE */
.sig-badge{display:inline-block;padding:3px 9px;border-radius:5px;font-size:8px;font-weight:700;letter-spacing:.8px;border:1px solid;font-family:'Space Mono',monospace;white-space:nowrap;text-transform:uppercase}

/* PACK GRID */
.pack-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px;padding:12px}

/* PACK CARD — premium refinement */
.pcard{background:#0d1220;border:1px solid rgba(255,255,255,.07);border-radius:14px;cursor:pointer;transition:all .2s cubic-bezier(.4,0,.2,1);position:relative;overflow:hidden}
.pcard:hover{border-color:rgba(255,255,255,.15);transform:translateY(-2px);box-shadow:0 12px 40px rgba(0,0,0,.6)}
.pcard.sel{border-color:rgba(0,255,136,.4);box-shadow:0 0 0 1px rgba(0,255,136,.15),0 12px 40px rgba(0,0,0,.5)}
.pcard-top{padding:13px 13px 10px}
.pcard-bot{border-top:1px solid rgba(255,255,255,.05);padding:10px 13px;background:rgba(0,0,0,.2)}
.pcard-stripe{position:absolute;top:0;left:0;right:0;height:2px}

/* BELL */
.bell{all:unset;cursor:pointer;width:28px;height:28px;border:1px solid rgba(255,255,255,.08);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0;transition:all .15s}
.bell:hover{border-color:rgba(255,255,255,.2);background:rgba(255,255,255,.04)}
.bell.on{border-color:rgba(255,209,102,.5);background:rgba(255,209,102,.08);animation:ap 2s infinite}
@keyframes ap{0%,100%{box-shadow:0 0 0 0 rgba(255,209,102,.3)}70%{box-shadow:0 0 0 5px rgba(255,209,102,0)}}

/* DETAIL PANEL */
.detail-panel{width:0;flex-shrink:0;border-left:1px solid rgba(255,255,255,.06);background:#0a0e16;display:flex;flex-direction:column;overflow:hidden;transition:width .22s cubic-bezier(.4,0,.2,1)}
.detail-panel.open{width:336px}
.dp-scroll{flex:1;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:8px}
.dp-sec{background:#0d1220;border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:12px;display:flex;flex-direction:column;gap:8px}
.dp-lbl{font-size:8px;letter-spacing:1.5px;color:rgba(255,255,255,.3);font-family:'Space Mono',monospace;text-transform:uppercase}
.dp-2{display:grid;grid-template-columns:1fr 1fr;gap:6px}
.dp-tile{background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.06);border-radius:8px;padding:9px 10px}
.dp-tl{font-size:8px;color:rgba(255,255,255,.3);letter-spacing:1px;display:block;margin-bottom:4px;font-family:'Space Mono',monospace;text-transform:uppercase}
.dp-tv{font-family:'Space Mono',monospace;font-weight:700;font-size:15px;line-height:1}
.fee-t{width:100%;font-size:10px;border-collapse:collapse;font-family:'Space Mono',monospace}
.fee-t td{padding:6px 0;border-bottom:1px solid rgba(255,255,255,.05);color:rgba(255,255,255,.4)}
.fee-t tr.tot td{color:#e8edf5;font-weight:700;border:none;padding-top:8px}
.dp-x{all:unset;cursor:pointer;color:rgba(255,255,255,.3);width:24px;height:24px;border:1px solid rgba(255,255,255,.08);border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:11px;flex-shrink:0;transition:all .15s}
.dp-x:hover{color:rgba(255,255,255,.75);border-color:rgba(255,255,255,.2)}

/* FEED CARD */
.feed-card{border:1px solid rgba(255,255,255,.07);border-radius:11px;padding:11px 13px;display:flex;gap:10px;align-items:flex-start;background:#0d1220;transition:background .15s}
.feed-card:hover{background:#111827}

/* BUDGET BUTTONS */
.b-btn{padding:6px 16px;border:1px solid rgba(255,255,255,.08);border-radius:20px;font-size:11px;cursor:pointer;background:transparent;color:rgba(255,255,255,.4);font-family:'Space Mono',monospace;transition:all .15s;font-weight:700}
.b-btn:hover{border-color:rgba(255,255,255,.2);color:rgba(255,255,255,.75)}
.b-btn.on{border-color:rgba(0,255,136,.35);color:#00ff88;background:rgba(0,255,136,.07)}

/* TOGGLE */
.toggle-wrap{display:flex;align-items:center;gap:10px;cursor:pointer}
.toggle{width:42px;height:24px;border-radius:12px;border:1px solid rgba(255,255,255,.1);background:rgba(0,0,0,.3);position:relative;transition:all .2s;flex-shrink:0}
.toggle.on{background:rgba(0,255,136,.15);border-color:rgba(0,255,136,.4)}
.toggle::after{content:'';position:absolute;top:3px;left:3px;width:16px;height:16px;border-radius:50%;background:rgba(255,255,255,.2);transition:all .2s}
.toggle.on::after{left:21px;background:#00ff88}
...
/* ALERT CARDS */
.atype-card{border:1px solid rgba(255,255,255,.07);border-radius:11px;padding:14px 16px;cursor:pointer;transition:all .15s;background:#0d1220}
.atype-card:hover{border-color:rgba(255,255,255,.15)}
.atype-card.on-green{border-color:rgba(0,255,136,.3);background:rgba(0,255,136,.04)}
...
.perm-btn{width:100%;padding:14px;border:none;border-radius:10px;font-weight:700;font-size:14px;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .15s}
.perm-btn:hover{filter:brightness(1.1);transform:translateY(-1px)}
.perm-btn:active{transform:none}

/* WATCH LIST */
.watch-pack{border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:12px;display:flex;align-items:center;gap:10px;cursor:pointer;transition:all .15s;background:#0d1220}
.watch-pack:hover{border-color:rgba(255,255,255,.15);background:#111827}
.watch-pack.on{border-color:rgba(255,209,102,.35);background:rgba(255,209,102,.04)}
...
.toast{background:#0d1220;border:1px solid rgba(0,255,136,.2);border-radius:10px;padding:12px 16px;pointer-events:all;max-width:300px;animation:toastin .25s ease;backdrop-filter:blur(8px)}
...
function Toast({msg,type,onClose}:{msg:string;type:"ok"|"warn"|"err";onClose:()=>void}){
  ...
  const colors={ok:"#00ff88",warn:"#ffd166",err:"#ff3860"};
  ...
  return(
    <div className="toast" style={{borderColor:`${colors[type]}44`}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1,fontSize:11,color:"#e8edf5",lineHeight:1.5}}>{msg}</div>
        <button onClick={onClose} style={{all:"unset" as any,cursor:"pointer",color:"rgba(255,255,255,.3)",fontSize:14,paddingLeft:8}}>✕</button>
      </div>
    </div>
  );
}
...
function AlertPanel({
  ...
  return(
    <>
      <div style={{position:"fixed",top:80,right:0,bottom:0,width:360,background:"#0a0e16",borderLeft:"1px solid rgba(255,255,255,.07)",zIndex:100,display:"flex",flexDirection:"column",transform:open?"translateX(0)":"translateX(100%)",transition:"transform .25s cubic-bezier(.4,0,.2,1)",boxShadow:open?"-4px 0 20px rgba(0,0,0,.4)":"none"}}>
        {/* Header */}
        <div style={{padding:"13px 16px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
          <div style={{flex:1}}>
            <div style={{fontWeight:800,fontSize:15,color:"#fff"}}>🔔 EV Alerts</div>
            <div style={{fontSize:9,color:"rgba(255,255,255,.3)",marginTop:2,...M}}>
              {isSubscribed?"Active — watching Courtyard every 60s":"Get notified when packs go +EV"}
            </div>
          </div>
          <button onClick={onClose} style={{all:"unset" as any,cursor:"pointer",color:"rgba(255,255,255,.3)",width:22,height:22,border:"1px solid rgba(255,255,255,.07)",borderRadius:4,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11}}>✕</button>
        </div>

        {/* Status / Enable */}
        <div style={{padding:"10px 14px",borderBottom:"1px solid rgba(255,255,255,.07)",flexShrink:0}}>
          {swStatus==="granted"&&isSubscribed?(
            <div style={{background:"rgba(0,255,135,.04)",border:"1px solid rgba(0,255,135,.18)",borderRadius:8,padding:"9px 12px",display:"flex",alignItems:"center",gap:9}}>
              ...
              <div>
                <div style={{fontWeight:700,fontSize:11,color:"#00ff88"}}>Notifications Active</div>
                <div style={{fontSize:9,color:"rgba(255,255,255,.3)",marginTop:1}}>Server checks Courtyard every 60s · alerts fire even when browser is closed</div>
              </div>
            </div>
          ):swStatus==="denied"?(
            <div style={{background:"rgba(255,56,96,.05)",border:"1px solid rgba(255,56,96,.2)",borderRadius:8,padding:"10px 12px"}}>
              ...
              <div style={{fontSize:9,color:"rgba(255,255,255,.3)",lineHeight:1.7,marginBottom:8}}>
              ...
              {[
                ...
                <div key={s.n} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:5}}>
                  ...
                  <span style={{fontSize:9,color:"#e8edf5",lineHeight:1.5}}>{s.t}</span>
                </div>
              ))}
              <div style={{display:"flex",gap:6,marginTop:8}}>
                ...
                <button onClick={()=>window.location.reload()}
                  style={{padding:"7px 12px",background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",borderRadius:6,fontSize:9,color:"rgba(255,255,255,.3)",cursor:"pointer",fontWeight:700,...M}}>
                  Refresh ↺
                </button>
              </div>
            </div>
            ...
            <div style={{background:"rgba(0,255,135,.04)",border:"1px solid rgba(0,255,135,.2)",borderRadius:8,padding:"10px 12px"}}>
              <div style={{fontWeight:700,fontSize:12,color:"#00ff88",marginBottom:5}}>⚡ Enable Push Notifications</div>
              <div style={{fontSize:9,color:"rgba(255,255,255,.3)",lineHeight:1.5,marginBottom:8}}>
              ...
              <button onClick={onEnableNotifs}
                style={{width:"100%",padding:"10px",background:"#00ff88",color:"#000",border:"none",borderRadius:7,fontWeight:800,fontSize:12,cursor:"pointer",...M}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"10px 14px",display:"flex",flexDirection:"column",gap:5}}>
          ...
          <div style={{background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",borderRadius:7,padding:"8px 10px",marginBottom:8}}>
            <div style={{fontSize:7,color:"rgba(255,255,255,.3)",letterSpacing:1.5,...M,marginBottom:5}}>YOU'LL GET AUTOMATICALLY</div>
            <div style={{display:"flex",flexDirection:"column" as const,gap:3}}>
              {[
                {e:"🔥",t:"Strong Buy",c:"#00ff88",s:"buyback ≥ 1.2x · profit after all fees"},
                ...
                <div key={a.t} style={{display:"flex",alignItems:"center",gap:6}}>
                  ...
                  <span style={{fontSize:9,color:"rgba(255,255,255,.3)"}}>{a.s}</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{fontSize:7,color:"rgba(255,255,255,.3)",letterSpacing:1.5,...M,marginBottom:4}}>
            SPECIFIC PACK ALERTS {alerts.size > 0 ? `(${alerts.size} selected)` : "(optional)"}
          </div>
          {packs.map(p=>{
            const isOn=alerts.has(p.id);
            const ec=p.evRatio>=1.3?"#00ff88":p.evRatio>=1?"#4fd8a0":p.evRatio>=0.9?"#ffd166":"#ff3860";
            const sg=p.evRatio>=1.3?"GREAT VALUE":p.evRatio>=1?"GOOD VALUE":p.evRatio>=0.9?"FAIR VALUE":"BELOW EV";
            const sgc=p.evRatio>=1.3?"#00ff88":p.evRatio>=1?"#4f8fff":p.evRatio>=0.9?"#ffd166":"#ff3860";
            return(
              <div key={p.id} onClick={()=>onToggle(p.id)}
                style={{border:`1px solid ${isOn?"rgba(0,255,135,.3)":"#122038"}`,borderRadius:8,padding:"9px 10px",cursor:"pointer",background:isOn?"rgba(0,255,135,.04)":"#0d1220",display:"flex",alignItems:"center",gap:9,transition:"all .15s"}}>
                ...
                {/* Toggle */}
                <div style={{width:36,height:20,borderRadius:10,border:`1px solid ${isOn?"rgba(0,255,135,.4)":"#122038"}`,background:isOn?"rgba(0,255,135,.12)":"#060608",position:"relative",flexShrink:0,transition:"all .2s"}}>
                  <div style={{position:"absolute",top:3,left:isOn?18:3,width:12,height:12,borderRadius:"50%",background:isOn?"#00ff88":"#3a5068",transition:"all .2s"}}/>
                </div>
              </div>
            );
          })}
        ...
        <div style={{padding:"8px 14px",borderTop:"1px solid rgba(255,255,255,.07)",flexShrink:0,textAlign:"center" as const}}>
...
function DetailPanel({sel,isPro,alerts,histData,onClose,onUpgrade,onToggleAlert}:{
  ...
  if(!isPro) return(
    <div className="si" style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{padding:"11px 12px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",gap:9,flexShrink:0,background:"#060608"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:9,color:"rgba(255,255,255,.3)",marginTop:1,...M}}>EV: {$x(sel.evRatio)} · ${sel.price}/pack</div>
        </div>
        ...
        {([
          ...
          <div key={f.t} style={{display:"flex",gap:10,alignItems:"center",width:"100%",maxWidth:260,textAlign:"left" as const}}>
            ...
            <div>
              ...
              <div style={{fontSize:9,color:"rgba(255,255,255,.3)"}}>{f.d}</div>
            </div>
            <span style={{marginLeft:"auto",color:"#00ff88",fontSize:12}}>✓</span>
          </div>
        ))}
        <button onClick={onUpgrade}
          style={{width:"100%",maxWidth:260,padding:"12px",background:"#00ff88",border:"none",borderRadius:10,fontWeight:800,fontSize:13,color:"#000",cursor:"pointer",fontFamily:"'Space Mono',monospace"}}>
          ⚡ Unlock — $29/mo Early Access →
        </button>
        <div style={{fontSize:9,color:"rgba(255,255,255,.3)",fontFamily:"'Space Mono',monospace"}}>$29/mo early access · First 100 users · Cancel anytime</div>
      </div>
    </div>
  );
  ...
  const evColor2=evc(p.evRatio), bbC=p.buybackEv>=1?"#00ff88":"#ff3860";
    ...
    <div className="si" style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{padding:"11px 12px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",gap:9,flexShrink:0,background:"#060608"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:9,color:"rgba(255,255,255,.3)",marginTop:1,...M}}>{p.categoryTitle??p.category} · ${p.price}/pack · {p.pullCount??p.totalPulls??0} pulls</div>
        </div>
      ...
      <div className="dp-scroll">
        ...
        <div className="dp-sec">
          ...
          <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"rgba(255,255,255,.3)",...M}}>
        ...
        <div className="dp-sec">
          ...
          <div className="dp-2">
            <div className="dp-tile" style={{borderColor:p.evRatio>=1?"rgba(0,255,135,.2)":"rgba(255,56,96,.15)"}}>
              ...
              <div style={{fontSize:8,color:"rgba(255,255,255,.3)",marginTop:3,...M}}>{$f(p.calEv??p.calibratedEv??0)} avg</div>
            </div>
            <div className="dp-tile" style={{borderColor:p.buybackEv>=1?"rgba(255,209,102,.2)":"rgba(255,56,96,.15)"}}>
              ...
              <div style={{fontSize:8,color:"rgba(255,255,255,.3)",marginTop:3,...M}}>{$f(cashOut)} cash</div>
            </div>
          </div>
          <div className="dp-2">
            <div className="dp-tile"><span className="dp-tl">WIN RATE</span><div className="dp-tv" style={{color:p.winRate>=.5?"#00ff88":"#3a5068"}}>{$p(p.winRate)}</div></div>
            <div className="dp-tile"><span className="dp-tl">AVG PULL VALUE</span><div className="dp-tv" style={{color:"#00ff88"}}>{$f(safeN(p.calEv??p.avgFmv))}</div></div>
          </div>
        ...
        <a href={`https://courtyard.io/vending-machine/${p.id}`} target="_blank" rel="noreferrer"
          style={{display:"block",textAlign:"center" as const,padding:"12px",borderRadius:9,fontWeight:800,fontSize:14,textDecoration:"none",background:p.evRatio>=1?"#00ff88":"#0d1220",color:p.evRatio>=1?"#000":"#3a5068",border:p.evRatio>=1?"none":"1px solid rgba(255,255,255,.07)"}}>
          {p.evRatio>=1?"🟢 Pull This Pack on Courtyard →":"View on Courtyard →"}
        </a>
        <div style={{fontSize:8,color:"rgba(255,255,255,.3)",textAlign:"center" as const,...M,lineHeight:1.6}}>For informational purposes only. Not financial advice.</div>
      </div>
    </div>
  );
}
...
function BestPackCard({pack}:{pack:Pack}){
  ...
  return(
    <div style={{background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",borderRadius:8,padding:"9px 12px",display:"flex",alignItems:"center",gap:11}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:7,color:"#00ff88",letterSpacing:1.5,...M,fontWeight:700,marginBottom:3}}>BEST VALUE RIGHT NOW</div>
        ...
        <div style={{display:"flex",gap:14}}>
          {([["EV",$x(pack.evRatio),"#00ff88"],["CAL. EV",$f(pack.calEv??0),"#00ff88"],["BUYBACK",$x(pack.buybackEv),pack.buybackEv>=1?"#00ff88":"#ff3860"]] as [string,string,string][]).map(([l,v,c])=>(
            <div key={l}><div style={{fontSize:7,color:"rgba(255,255,255,.3)",...M}}>{l}</div><div style={{fontWeight:800,fontSize:13,color:c,...M}}>{v}</div></div>
          ))}
        </div>
      </div>
      <span className="sig-badge" style={{color:sg.c,background:sg.bg,borderColor:sg.bd,alignSelf:"flex-start" as const}}>{sg.label}</span>
...
function Dashboard(){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#060608",color:"#e8edf5"}}>
      <style>{CSS}</style>
            ...
            <div className="ticker-track">
              {[...data.recentPulls,...data.recentPulls].map((p,i)=>{
                ...
                return(<div key={`${p.id}-${i}`} className="ticker-item">
                  <span style={{color:"rgba(255,255,255,.3)",fontSize:9}}>@{name}</span>
                  <span style={{color:"#e8edf5",fontWeight:600}}>{title.slice(0,20)}</span>
                  <span style={{color:win?"#00ff88":"#ff3860",fontWeight:800}}>{$f(fmv)}</span>
      ...
      <nav className="nav">
        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          ...
          <div>
            ...
            <div style={{fontSize:7,color:"rgba(255,255,255,.3)",letterSpacing:1.5,...M}}>COURTYARD DECISION ENGINE</div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:5,marginLeft:10,padding:"2px 10px",border:"1px solid rgba(0,255,135,.2)",borderRadius:20,background:"rgba(0,255,135,.04)",flexShrink:0}}>
          ...
          <span style={{fontSize:8,color:"#00ff88",fontWeight:700,letterSpacing:1,...M}}>{data?.isLive?"LIVE":"SAMPLE"} · ↻{cd}s</span>
        </div>
        {data&&[
          {l:"+EV PACKS",v:`${data.posEV}/${data.packs.length}`,c:"#00ff88"},
          {l:"AVG EV",v:$x(data.avgEV),c:data.avgEV>=1?"#00ff88":"#ff3860"},
        ...
        <button onClick={()=>refetch()} disabled={isLoading} style={{marginLeft:"auto",padding:"4px 11px",background:"transparent",border:"1px solid rgba(255,255,255,.07)",borderRadius:5,fontSize:8,color:"rgba(255,255,255,.3)",cursor:"pointer",...M}}>↻ Refresh</button>
        {/* Auth */}
        {authLoaded&&(user?(
          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            {!isPro&&<button onClick={()=>setShowUpgrade(true)} style={{padding:"4px 10px",background:"rgba(0,255,135,.1)",border:"1px solid rgba(0,255,135,.3)",borderRadius:5,fontSize:9,color:"#00ff88",cursor:"pointer",fontWeight:700,...M}}>⚡ Upgrade</button>}
            <UserButton afterSignOutUrl="/" appearance={{elements:{userButtonAvatarBox:"w-7 h-7"}}}/>
          </div>
        ):(
          <SignInButton mode="modal">
            <button style={{padding:"4px 12px",background:"#00ff88",border:"none",borderRadius:5,fontSize:9,color:"#000",cursor:"pointer",fontWeight:800,...M}}>Sign In</button>
          </SignInButton>
        ))}
      </nav>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        {/* Sidebar */}
        <aside style={{width:210,flexShrink:0,borderRight:"1px solid rgba(255,255,255,.07)",background:"#0a0e16",display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{padding:"9px 10px 3px",fontSize:7,color:"rgba(255,255,255,.3)",letterSpacing:1.5,...M}}>WORKSPACE</div>
          {([
            ["packs","⚡","EV Terminal"],
            ...
            <button key={v} className={`sb-btn${view===v?" on":""}`} onClick={()=>setView(v as any)}
              style={{borderLeftColor:view===v?"#00ff88":"transparent",background:view===v?"rgba(255,255,255,.025)":"transparent",color:view===v?"#c8dff0":"#3a5068"}}>
          ...
          <button className={`sb-btn${alertsOpen?" on":""}`} onClick={()=>isPro?setAlertsOpen(true):setShowUpgrade(true)}
            ...
            {isSubscribed
              ? <span style={{flexShrink:0,fontSize:7,fontWeight:700,color:"#00ff88",background:"rgba(0,255,135,.12)",padding:"2px 5px",borderRadius:3,...M}}>LIVE</span>
          ...
          <div style={{padding:"9px 10px 3px",marginTop:4,fontSize:7,color:"rgba(255,255,255,.3)",letterSpacing:1.5,...M}}>MARKETS</div>
          {cats.map(c=>{
            const color=c==="all"?"#00ff88":CAT_COLOR[c]??"#888";
            ...
            return(
              <button key={c} className={`sb-btn${active?" on":""}`} onClick={()=>{setTab(c);setView("packs");}}
                ...
                {buyN>0&&<span style={{flexShrink:0,fontSize:8,fontWeight:700,color:"#00ff88",background:"rgba(0,255,135,.08)",padding:"1px 4px",borderRadius:3,...M}}>{buyN}↑</span>}
              </button>
            );
          })}
          <div style={{marginTop:"auto",padding:10,borderTop:"1px solid rgba(255,255,255,.07)"}}>
            <a href="https://courtyard.io" target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:6,fontSize:10,color:"rgba(255,255,255,.3)",textDecoration:"none"}}>🔗 Open Courtyard.io</a>
          </div>
        </aside>
        ...
        <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          {isLoading&&(<div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12}}>
            ...
            <div style={{fontSize:13,fontWeight:700,color:"rgba(255,255,255,.3)"}}>Loading live Courtyard data...</div>
          </div>)}
          ...
          {data&&view==="packs"&&(
            <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
              {/* Hero */}
              <div style={{padding:"10px 14px",borderBottom:"1px solid rgba(255,255,255,.07)",background:"#0a0e16",flexShrink:0}}>
                <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:10}}>
                  <div style={{flex:1}}>
                    <h1 style={{fontWeight:800,fontSize:20,color:"#fff",lineHeight:1.1,letterSpacing:-0.5}}>Stop buying packs blind.</h1>
                    <p style={{fontSize:11,color:"rgba(255,255,255,.3)",marginTop:3}}>Real-time EV + real cash after fees for every <a href="https://courtyard.io" target="_blank" rel="noreferrer" style={{color:"#00ff88"}}>Courtyard.io</a> pack.</p>
                  </div>
                  {best&&<a href={`https://courtyard.io/vending-machine/${best.id}`} target="_blank" rel="noreferrer" style={{padding:"10px 20px",background:"#00ff88",borderRadius:10,color:"#000",fontWeight:800,fontSize:13,textDecoration:"none",flexShrink:0}}>Rip a Pack ↗</a>}
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr 2fr",gap:8}}>
                  {[
                    {l:"+EV PACKS",v:`${data.posEV}/${data.packs.length}`,s:"above break-even",vc:"#00ff88"},
                    {l:"BEST EV NOW",v:$x(data.bestPack?.evRatio??0),s:data.bestPack?.name??"—",vc:"#00ff88"},
                    {l:"PULLS TRACKED",v:`${data.totalPulls}`,s:data.isLive?"live data":"sample",vc:"#fff"},
                    {l:"MARKET AVG EV",v:$x(data.avgEV),s:data.avgEV>=1?"Market healthy":"Below EV",vc:data.avgEV>=1?"#00ff88":"#ff3860"},
                  ].map(s=>(
                    <div key={s.l} style={{background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",borderRadius:8,padding:"8px 11px"}}>
                      <div style={{fontSize:7,color:"rgba(255,255,255,.3)",letterSpacing:1.2,...M,marginBottom:3}}>{s.l}</div>
                      <div style={{fontWeight:800,fontSize:18,color:s.vc,lineHeight:1,...M}}>{s.v}</div>
                      <div style={{fontSize:9,color:"rgba(255,255,255,.3)",marginTop:3}}>{s.s}</div>
                    </div>
                  ))}
              ...
              {/* Category tabs */}
              <div style={{display:"flex",alignItems:"center",gap:6,padding:"7px 12px",borderBottom:"1px solid rgba(255,255,255,.07)",background:"#0a0e16",flexShrink:0,overflowX:"auto" as const}}>
                {cats.map(c=>{
                  const packs=data?(c==="all"?data.packs:data.packs.filter(p=>p.category===c)):[];
                ...
                <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:5,flexShrink:0}}>
                  <span className="ld" style={{width:5,height:5}}/><span style={{fontSize:8,color:"#00ff88",fontWeight:700,...M}}>LIVE</span>
                </div>
              </div>

              {/* Toolbar */}
              <div style={{display:"flex",alignItems:"center",gap:4,padding:"5px 12px",borderBottom:"1px solid rgba(255,255,255,.07)",background:"#0a0e16",flexShrink:0,flexWrap:"wrap" as const}}>
                ...
                <span style={{fontSize:8,color:"rgba(255,255,255,.3)",marginLeft:8,...M}}>SORT</span>
              ...
              <div style={{flex:1,overflowY:"auto"}}>
                <div className="pack-grid">
                  {rows.map((pack,idx)=>{
                    ...
                    const bbC=pack.buybackEv>=1?"#00ff88":"#ff3860";
                      ...
                      <div key={pack.id} className={`pcard fi${isSel?" sel":""}`} onClick={()=>setSel(isSel?null:pack)}>
                        ...
                        <div className="pcard-top">
                          <div style={{display:"flex",alignItems:"flex-start",gap:9,marginBottom:10}}>
                            <div style={{width:44,height:58,borderRadius:7,overflow:"hidden",background:"#060608",border:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                            ...
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:8,color:"rgba(255,255,255,.3)",marginBottom:2,...M}}>
                              ...
                              <div style={{fontSize:10,color:"rgba(255,255,255,.3)"}}>${pack.price}.00 · {pack.pullCount??pack.totalPulls??0} pulls</div>
                            </div>
                          ...
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                            <div style={{background:"#060608",borderRadius:6,padding:"7px 9px",border:`1px solid ${pack.evRatio>=1?"rgba(0,255,135,.15)":"rgba(255,56,96,.1)"}`}}>
                              <div style={{fontSize:7,color:"rgba(255,255,255,.3)",...M,marginBottom:2}}>EV RATIO</div>
                              <div style={{fontWeight:800,fontSize:20,color:evColor2,lineHeight:1,...M}}>{$x(pack.evRatio)}</div>
                              <div style={{fontSize:8,color:"rgba(255,255,255,.3)",marginTop:2}}>{pack.evRatio>=1?"▲ above":"▼ below"} break-even</div>
                            </div>
                            <div onClick={(e)=>{if(!isPro){e.stopPropagation();setShowUpgrade(true);}}} style={{background:"#060608",borderRadius:6,padding:"7px 9px",border:`1px solid ${isPro&&pack.buybackEv>=1?"rgba(255,209,102,.15)":"rgba(255,56,96,.1)"}`,position:"relative",cursor:isPro?"default":"pointer"}}>
                              ...
                              {isPro?(
                                <>
                                  ...
                                  <div style={{fontSize:8,color:"rgba(255,255,255,.3)",marginTop:2}}>≈{$f((pack.calEv??pack.avgFmv??0)*0.846)} cash</div>
                                </>
                              ):(
                                <>
                          ...
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                            <div>
                              <div style={{fontSize:7,color:"rgba(255,255,255,.3)",...M,marginBottom:2}}>CAL. EV</div>
                              <div style={{fontWeight:700,fontSize:14,color:evColor2,...M}}>{$f(pack.calEv??pack.calibratedEv??0)}</div>
                            </div>
                            <div style={{textAlign:"right" as const}}>
                              <div style={{fontSize:8,color:"rgba(255,255,255,.3)",marginBottom:3}}>Win rate <span style={{color:"#e8edf5",fontWeight:700,...M}}>{$p(pack.winRate)}</span></div>
                              <div style={{width:90,height:3,borderRadius:2,background:"#0e1e30",overflow:"hidden",marginLeft:"auto"}}>
                                <div style={{height:"100%",borderRadius:2,width:`${Math.min(pack.winRate*200,100)}%`,background:pack.evRatio>=1?"#00ff88":"#ffd166"}}/>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="pcard-bot">
                          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:6}}>
                              ...
                              <a href={`https://courtyard.io/vending-machine/${pack.id}`} target="_blank" rel="noreferrer"
                                ...
                                style={{padding:"3px 9px",border:"1px solid rgba(0,255,135,.25)",borderRadius:5,fontSize:9,color:"#00ff88",fontWeight:700,textDecoration:"none",...M}}>
                                View ↗
                              </a>
                            </div>
                          </div>
                          {isPro?(
                            ...
                            <div onClick={(e)=>{e.stopPropagation();setShowUpgrade(true);}} style={{background:"rgba(0,255,135,.04)",border:"1px solid rgba(0,255,135,.15)",borderRadius:6,padding:"6px 8px",display:"flex",alignItems:"center",gap:8,cursor:"pointer"}}>
                              <span style={{fontWeight:800,fontSize:11,color:"#00ff88",...M,flexShrink:0}}>⚡ UPGRADE</span>
                              <span style={{fontSize:9,color:"rgba(255,255,255,.3)",lineHeight:1.3}}>Unlock BUY/WAIT/SKIP signals + Buyback EV</span>
                            </div>
                          )}
                        </div>
                      </div>
          ...
          {data&&view==="budget"&&(!isPro?(
            <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12,padding:24,textAlign:"center" as const}}>
              ...
              <div style={{fontSize:12,color:"rgba(255,255,255,.3)",maxWidth:320,lineHeight:1.6}}>
              ...
              {([
                ...
                <div key={f.t} style={{display:"flex",gap:10,alignItems:"center",width:"100%",maxWidth:280,textAlign:"left" as const,background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",borderRadius:8,padding:"8px 12px"}}>
              ...
              <button onClick={()=>setShowUpgrade(true)}
                style={{width:"100%",maxWidth:280,padding:"12px",background:"#00ff88",border:"none",borderRadius:10,fontWeight:800,fontSize:13,color:"#000",cursor:"pointer",fontFamily:"'Space Mono',monospace",marginTop:4}}>
                ⚡ Unlock — $29/mo Early Access →
              </button>
              <div style={{fontSize:9,color:"rgba(255,255,255,.3)",fontFamily:"'Space Mono',monospace"}}>First 25 users · Cancel anytime · 7-day refund</div>
            </div>
          ):(
            <div style={{flex:1,overflowY:"auto",padding:16}}>
              <h2 style={{fontWeight:800,fontSize:20,color:"#fff",marginBottom:6}}>💰 Budget Advisor</h2>
              <p style={{fontSize:12,color:"rgba(255,255,255,.3)",lineHeight:1.6,maxWidth:560,marginBottom:16}}>Tell us your budget. We find the best pack ranked by real buyback EV — actual cash after all fees.</p>
              ...
              {(budget?[budget]:[10,25,50,100]).map(b=>{
                ...
                return(
                  <div key={b} style={{background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:16,marginBottom:12}}>
                    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
                      <div style={{fontSize:26,fontWeight:800,color:"#00ff88",...M}}>${b}</div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:9,color:"rgba(255,255,255,.3)",...M,marginBottom:2}}>BEST PICK FOR THIS BUDGET</div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:10}}>
                      {[["EV RATIO",$x(top.evRatio),top.evRatio>=1?"#00ff88":"#ff3860"],["BUYBACK EV",$x(top.buybackEv),top.buybackEv>=1?"#00ff88":"#ff3860"],["CASH OUT",$f(safeN(top.calEv??top.avgFmv)*0.846),top.buybackEv>=1?"#00ff88":"#ff3860"],["WIN RATE",$p(top.winRate),top.winRate>=.5?"#00ff88":"#3a5068"]].map(([l,v,c])=>(
                        <div key={l as string} style={{background:"#060608",borderRadius:7,padding:"8px 10px",border:"1px solid rgba(255,255,255,.07)"}}>
                          <div style={{fontSize:7,color:"rgba(255,255,255,.3)",...M,marginBottom:3}}>{l}</div>
                    ...
                    {recs.length>1&&(
                      <div>
                        <div style={{fontSize:8,color:"rgba(255,255,255,.3)",...M,marginBottom:6}}>OTHER OPTIONS FOR ${b}</div>
                        <div style={{display:"flex",gap:8,flexWrap:"wrap" as const}}>
                          {recs.slice(1).map(p=>{const pd=dec(p);return(
                            <div key={p.id} style={{background:"#060608",border:"1px solid rgba(255,255,255,.07)",borderRadius:8,padding:"7px 10px"}}>
                              <div style={{fontWeight:700,color:"#e8edf5",marginBottom:2,fontSize:11}}>{p.name}</div>
          ...
          {data&&view==="feed"&&(
            <div style={{flex:1,overflowY:"auto",padding:10,display:"flex",flexDirection:"column",gap:6}}>
              <div style={{padding:"4px 2px 6px",fontSize:11,color:"rgba(255,255,255,.3)"}}>{data.recentPulls.length} most recent pulls from Courtyard.io — live feed</div>
              {(isPro ? data.recentPulls : data.recentPulls.slice(0,5)).map((pull,_pullIdx)=>{
                ...
                return(
                  <div key={pull.id} className="feed-card" style={{alignItems:"center"}}>
                    {/* Card image */}
                    {cardImg?(
                      <img src={cardImg} alt="" style={{width:44,height:60,objectFit:"contain",borderRadius:4,flexShrink:0,background:"#0d1220"}}
                        onError={(e)=>{(e.target as HTMLImageElement).style.display="none";}}/>
                    ):(
                      <div style={{width:44,height:60,borderRadius:4,background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"rgba(255,255,255,.3)",...M}}>IMG</div>
                    )}
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap" as const}}>
                        ...
                        {packObj&&<span style={{fontSize:8,color:"rgba(255,255,255,.3)",padding:"1px 5px",background:"#0d1220",border:"1px solid rgba(255,255,255,.07)",borderRadius:3,...M}}>{packObj.name}</span>}
                        {timeStr&&<span style={{fontSize:8,color:"rgba(255,255,255,.3)",marginLeft:"auto",...M}}>{ago(timeStr)} ago</span>}
                      </div>
                      ...
                      {pull.grade&&<div style={{fontSize:8,color:"rgba(255,255,255,.3)",marginTop:2,...M}}>{pull.grade}</div>}
                    </div>
                    <div style={{textAlign:"right" as const,flexShrink:0}}>
                      <div style={{fontWeight:800,fontSize:14,color:fmv>0?(win?"#00ff88":"#ff3860"):"#3a5068",...M}}>{fmv>0?$f(fmv):"—"}</div>
      ...
      <div style={{padding:"4px 14px",borderTop:"1px solid rgba(255,255,255,.07)",background:"#0a0e16",textAlign:"center" as const}}>
        <span style={{fontSize:9,color:"rgba(255,255,255,.3)"}}>For educational purposes only. Not financial advice. Pack outcomes are random. Not affiliated with Courtyard.io.</span>
      </div>
    </div>
  );
}
...
function AppRouter(){
  ...
  if(!isLoaded) return(
    <div style={{height:"100vh",background:"#030810",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{fontFamily:"'Space Mono',monospace",color:"#00ff88",fontSize:12,letterSpacing:2}}>LOADING...</div>
    </div>
  );
  ...
  return(
    <div style={{height:"100vh",background:"#030810",display:"flex",flexDirection:"column" as const,alignItems:"center",justifyContent:"center",gap:20,fontFamily:"'Space Mono',monospace"}}>
      <div style={{fontSize:28,fontWeight:800,color:"#fff",fontFamily:"Syne,sans-serif"}}>Sign in to continue</div>
      <div style={{fontSize:13,color:"rgba(255,255,255,.3)",marginBottom:8}}>PackPulse requires an account to access live EV data.</div>
      <SignInButton mode="modal" forceRedirectUrl="/?app=1">
        <button style={{padding:"13px 32px",background:"#00ff88",color:"#000",border:"none",borderRadius:9,fontWeight:800,fontSize:14,cursor:"pointer",fontFamily:"'Space Mono',monospace"}}>
          Sign In / Create Account →
        </button>
      </SignInButton>
      <button onClick={()=>setShowApp(false)} style={{all:"unset" as any,cursor:"pointer",fontSize:12,color:"rgba(255,255,255,.3)"}}>
        ← Back to home
      </button>
    </div>
  );
